...
#define CACHE_BYTES (1u << 19)
#define CACHE_BLOCK 32
#define MIN_TICKS 1000
#define MIN_REPS 8

static unsigned long int kbest = K;
static bool clear_cache = CLEAR_CACHE;
static unsigned long int maxsamples = MAXSAMPLES;
static double epsilon = EPSILON;
static unsigned long int cache_bytes = CACHE_BYTES;
static unsigned long int cache_block = CACHE_BLOCK;
static unsigned long int min_reps = MIN_REPS;
static unsigned long int min_ticks = MIN_TICKS;
static double min_time = 0;

static unsigned long int *cache_buf = NULL;
...
static void init_min_time(void) {
    if (min_time == 0.0)
        min_time = (double)min_ticks * timer_resolution;
}
...
static unsigned long int has_converged(void) {
...
static volatile unsigned long int sink = 0;

static void clear(void) {
    unsigned long int x = sink;
    unsigned long int *cptr, *cend;
    unsigned long int incr = cache_block / sizeof(long int);
    if (!cache_buf) {
    ...
    cptr = cache_buf;
    cend = cptr + cache_bytes / sizeof(unsigned long int);
    while (cptr < cend) {
        x += *cptr;
        cptr += incr;
    }
...
double fcyc(test_funct f, void *args) {
    ...
    do {
        ...
        cyc = (double)get_counter() / (double)reps;
        if (cyc > 0.0)
            add_sample(cyc);
...
double fsec(test_funct f, void *args) {
    double result;
    /* Increase reps until we get meaningful times */
    unsigned long reps = min_reps;
    unsigned long r;
    double sec = 0.0;
    init_min_time();
...
/* Maximum number of samples attempting to find K-best within some tolerance.
   ...
   Default = 20
*/
void set_fcyc_maxsamples(unsigned long int maxsamples_arg) {
